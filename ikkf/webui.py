...
#cy {{ width: 100%; height: 600px; background: #0d1117; border-radius: 8px; border: 1px solid #30363d; }}
...
<a href="/graph" class="{'on' if page=='graph' else ''}">Граф связей</a>
...
async def monitor_page():
    html = """<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Agent Smith - Monitor</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
<h1>Agent Smith - Real-time Monitor</h1>
...
async function update(){
  try{
    ...
    svc.innerHTML='';
    for(let[n,v] of Object.entries(d.services)){
      let c=v==='running'?'ok':(v==='partial'?'warn':'err');
      svc.innerHTML+='<div class="card"><div class="label">'+n+'</div><div class="value '+c+'">'+v+'</div></div>';
    }
    let sys=document.getElementById('system');
    sys.innerHTML='';
    for(let[n,v] of Object.entries(d.system)){
      sys.innerHTML+='<div class="card"><div class="label">'+n+'</div><div class="value">'+v+'</div></div>';
    }
  }catch(e){}
}
update();
...
async def graph_page():
    stats = api_call("GET", "/stats")
    html = base_header("graph", stats)

    # Получаем последние 50 узлов и их связи
    nodes_data = api_call("GET", "/nodes", params={"limit": 50})
    ...
    # Для каждого узла ищем соседей
    for nid in list(node_ids):
        neighbors_data = api_call("GET", f"/neighbors/{nid}")
        neighbors = neighbors_data.get("neighbors", [])
        for nb in neighbors:
            target_id = nb["node"]["id"]
            # Добавляем связь только если целевой узел тоже в нашем списке
            if target_id in node_ids:
                elements.append({
                    "data": {
                        "id": f"e_{nid}_{target_id}",
                        "source": nid,
                        "target": target_id,
                        "label": nb["edge"]["edge_type"]
                    }
                })

    html += f"""
    <div class="card">
        <h2>Интерактивный граф знаний</h2>
        <div id="cy"></div>
    </div>
    <script>
        var cy = cytoscape({{
